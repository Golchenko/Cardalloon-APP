  ...
  const getTestEndpoint = async () => {
    ...
    try {
      ...
      if(responseData && responseData.data){
        ...
        if(responseData.data.orders && responseData.data.customers){
          setShopInventory({
...
const RegisteredMerchant = (props) => {
  ...
  const rows = props.shopInventory.orders.map((order, index) => {
      return [index+1, order.name, order.status, order.tags, new Date(order.created_at).toUTCString(), new Date(order.updated_at).toUTCString()]
  ...
  return(
    <Page 
      singleColumn title={` Welcome ${props.currentUser.first_name}`}
      // primaryAction={{
      //   content: "Synchronise with 60",
      //   onAction: indexPagePrimaryAction
      // }}
